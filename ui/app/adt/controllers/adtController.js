    ...
    .controller('AdtController', ['$scope', '$q', '$rootScope', 'spinner', 'dispositionService', 'encounterService', 'bedService', 'appService', 'visitService', '$location', '$window', 'sessionService', 'messagingService',
        function ($scope, $q, $rootScope, spinner, dispositionService, encounterService, bedService, appService, visitService, $location, $window, sessionService, messagingService) {
            ...
            var initializeActionConfig = function(){
                ...
                var cancelDischargeActions = appService.getAppDescriptor().getExtensions("org.bahmni.adt.cancel.discharge.action", "config");
                if (encounterConfig) {
                    ...
                    actionConfigs[Constants.cancelDischargeCode] = {encounterTypeUuid: encounterConfig.getDischargeEncounterTypeUuid(), allowedActions: cancelDischargeActions};
                }
            };
            ...
            var getDispositionActions = function (actions) {
                if ($scope.isAdmitted()) {
                    ...
                    return filterAction(actions, 'Cancel Discharge');
                } else {
            ...
            $scope.cancelDischarge = function () {
                ...
                spinner.forPromise(encounterService.delete(encounterData.uuid, "Cancelling Discharge")).success(function (response) {
                    forwardUrl(response, "onAdmissionForwardTo");
                });
            };
